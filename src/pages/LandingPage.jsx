import { CornerUpRight, TrendingUp, Clock } from "lucide-react"; 
...
const LandingPage = () => {
  ...
  <div
    className={`relative p-8 md:p-12 text-white z-10`}
  >
    <h2 className="text-3xl md:text-4xl font-extrabold mb-2 font-poppins">
      Your Campus Knowledge Hub.
    </h2>

    <p className="text-lg mb-6 opacity-80 font-roboto">
      Connect with BDU students and faculty. Ask, answer, and learn together.
    </p>

    <button
      onClick={() => navigate("/ask-question")}
      className="px-6 py-3 text-lg font-bold rounded-xl shadow-md transition-transform transform hover:scale-[1.02]"
      style={{ backgroundColor: BDU.GOLD, color: BDU.NAVY }}
    >
      Ask a Question Now <CornerUpRight size={18} className="inline ml-1" />
    </button>
  </div>
</div>
